...
function radiovisplayer_initsocket(topic, host, port) {
	...
	elem.shadowRoot.querySelector('.radiovis-innertext').innerHTML = "Connecting to " + topic + "...";
	...
	{ 
		...
		//var elem = document.querySelector('.radiovis-main[topic="' + evt.target.radiovis_topic + '"]');
		...
		if (splited_message[0] == 'RADIOVISWEBSOCKET') {  // Internal message
			if (splited_message[1] == 'HELLO\x00') {
				...
				elem.querySelector(".radiovis-innertext").innerHTML = radiovisplayer_connected_message;
				return;
			}
			if (splited_message[1] == 'ERROR') {
				//elem.find('.radiovis-innertext').html('<span style="color: red;">Error: ' + splited_message[2] + '</span>');
				elem.querySelector(".radiovis-innertext").innerHTML = '<span style="color: red;">Error: ' + splited_message[2] + '</span>';
				return;
			}
			elem.querySelector(".radiovis-innertext").innerHTML = '<span style="color: red;">Unexcepted reply: ' + splited_message[1] + '</span>';
			...
			if (command == 'MESSAGE') {
				...
				if (headers['destination']  == el.topic + '/text') {
					...
					if (body.substr(0, 5) == 'TEXT ') {
						...
						elem.querySelector('.radiovis-innertext').innerHTML = body.substr(5);
					}
				}
				...
				if (headers['destination']  == el.topic + '/image') {
					...
					if (body.substr(0, 5) == 'SHOW ') {
						...
						if (el.hasAttribute('lastImageShown'))
							lastShow = elem.getAttribute('lastImageShown');
						else 
							lastShow = '1';
						...
						elem.querySelector('.radiovis-I' + toShow).setAttribute('src', body.substr(5));
						...
						if(toShow == 1) {
							...
							fadeIn(elem);
							//elem.querySelector('.radiovis-I0').hide();
							//elem.querySelector('.radiovis-I1').fadeIn(1000);
						}
						else {
						...
						elem.setAttribute('lastImageShown', toShow);

						//Some radio dosen't send text. If we're still on the default message, remove it
						if (elem.querySelector('.radiovis-innertext').innerHTML == radiovisplayer_connected_message)
							elem.querySelector('.radiovis-innertext').innerHTML = "";

						//Update link
						if (headers['link'])
							elem.querySelector('a').setAttribute('href', headers['link']);
						else
	...
	{ 
		...
		//let query = '[topic="' + topic + '"]';
		//let elem = document.querySelector(query);
		el.shadowRoot.querySelector('.radiovis-innertext').innerHTML = "Connection lost, reconnecting...";
		setTimeout("radiovisplayer_initsocket('" + evt.target.topic + "', '" + evt.target.host + "', " + evt.target.port + ");", 1000)
	};
...
class Frame extends HTMLElement
{
	constructor(){
		...
		div.classList.add("radio-mainframe");
		div.innerHTML = `
		    <div class="radiovis-slideframe"
		        <div class="radiovis-P1"><a href="" class="radiovis-LI"><img class="radiovis-I1" src="' + radiovisplayer_first_image + '"></a></div>
		        <div class="radiovis-P0"><a href="" class="radiovis-LI"><img class="radiovis-I0" src="' + radiovisplayer_first_image + '"></a></div>
		    </div>
		`
		let div2 = document.createElement("div");
		div2.classList.add("radiovis-outtertext");
		div2.innerHTML = "<div class='radiovis-innertext'>Initialization...</div>";
	...
	connectedCallback() {
		...
		//console.log(this);
		//radiovisplayer_initsocket(this.topic, this.host, this.port);
	...
	initVisSocket(el) {
		...
		{ 
			...
			//let query = '[topic="' + topic + '"]';
			//let elem = document.querySelector(query);
			//self.shadowRoot.querySelector('.radiovis-innertext').innerHTML = "Connection lost, reconnecting...";
			console.log(el);
...
(function( $ ) {
	$.fn.radiovisplayer = function(topic, host, port) {
		...
		//Add properties
		this.setAttribute('radiovis-topic', topic);
